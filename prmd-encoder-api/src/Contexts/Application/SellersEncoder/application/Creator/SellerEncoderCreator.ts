...
import { CreateSellerEncoderRequest } from './CreateSellerEncoderRequest';
...
import { SellerSecuentialIncrementer } from '../../../Shared/domain/SellerEncoder/SellersSecuential';
import { PromedApiError } from '../../../Shared/domain/PromedApiError';

export class SellerEncoderCreator {
  constructor() {
  }

  async run(request: CreateSellerEncoderRequest) {

    const existSellers = await HttpSellerRepository();

    if (!existSellers.length) {
    ...
    SellerSecuentialIncrementer();

    return new CreateSellerEncoderResponse(sellerEncoder);
  }
}
